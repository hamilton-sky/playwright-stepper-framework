...
from stepper.resolvers.strategies import SemanticResolver, VisualAIResolver, DescriptionFallbackResolver
from stepper.resolvers.ai_pick_resolver import AIPickResolver
...
class ElementResolver:
    ...
    async def resolve(
        ...
        # Zero-selector mode: no recognized element keys → use description fallback
        if not cfg or not any(k in cfg for k in self._SELECTOR_KEYS):
            if step_description:
                logger.info(
                    f"[ElementResolver] no element keys in cfg — "
                    f"using description fallback: '{step_description[:60]}'"
                )
                loc, conf = await self._desc_fallback.find(page, step_description)
                if loc is not None:
                    return ResolveResult(
                        found=True, locator=loc,
                        confidence=conf, method="description-fallback"
                    )
                logger.warning("[ElementResolver] description fallback found nothing → visual AI")
        ...
        # All deterministic strategies failed → visual AI
        logger.warning("Deterministic cascade failed → visual AI fallback")
        return await self._visual_fallback(page, cfg, step_description)
...
class DefaultResolverFactory(ResolverFactory):
    ...
    def build_cascade(self):
        from stepper.resolvers.strategies import (
