...
from shared_poms.utils.book_filter import extract_year_from_text, is_under_year
...
class BookSearchPage(BasePage):
    ...
    class Locators:
        ...
        SEARCH_INPUT_CFGS: list[dict] = [
            {"css": "input#q",              "priority": 10},
            {"css": "input[name='q']",      "priority": 20},
            {"css": "input[type='search']", "priority": 30},
        ]

        SEARCH_BUTTON = "button[type='submit']"
        SEARCH_BUTTON_CFGS: list[dict] = [
            {"css": "button[type='submit']", "priority": 10},
        ]

    def __init__(self, driver, base_url: str, delays=None):
        super().__init__(driver, base_url, delays)
        self._query    = ""
        self._page_num = 1
