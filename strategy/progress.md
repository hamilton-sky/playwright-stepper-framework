...
| [ ] | 6 | Q9 | Rename `_resolve_context_vars` | CS-4 |
| [ ] | 7 | E3 | Reuse browser across `--data` rows | CS-5 |
...
| [ ] | 10 | Q5 | Extract env-resolution preamble helper | CS-4 |
| [ ] | 11 | Q10 | Extract single `_notify` helper | CS-4 |
| [ ] | 12 | Q6 | Add `press_enter` opt-out to `FillAction` | CS-4 |
| [ ] | 13 | Q12 | Extract `_fetch_attr` helper | CS-4 |
| [ ] | 14 | E1 | Move `_load_env()` inside `main()` | CS-5 |
| [ ] | 15 | E2 | Build `ElementResolver` once per session | CS-5 |
| [ ] | 16 | E5 | Single `mkdir` for `screenshots_dir` | CS-5 |
| [ ] | 17 | E8 | Guard `deepcopy` with `{{` token scan | CS-6 |
| [ ] | 18 | E9 | Add per-step `skip_screenshot` flag | CS-6 |
...
| Status | # | ID | Description | Change Set |
|--------|---|----|-------------|------------|
| [ ] | 21 | R3 | Use `CONFIDENCE_AUTO` in `is_high_confidence` | CS-7 |
| [x] | 22 | R5 | Hoist `import json` to module level | CS-3 |
| [x] | 23 | Q1 | Remove redundant `cfg_headless` variable | CS-2 |
| [ ] | 24 | Q7 | Remove dead `if step.extra else None` guards | CS-4 |
| [ ] | 25 | Q11 | Fix `ExecutionContext` docstring | CS-7 |
| [ ] | 26 | Q13 | Add `TypeVar` to `GlueAction._build_pom` | CS-7 |
| [x] | 27 | R2 | Deduplicate `allure serve` call | CS-2 |
| [ ] | 28 | R8 | Adopt `python-dotenv` | CS-7 |
| [ ] | 29 | E4 | Guard `storage_state` write | CS-6 |
| [ ] | 30 | E7 | Move `mkdir` to `MeasurePerformanceAction.__init__` | CS-6 |
...
| CS-4 — strategies.py quality | Q5, Q10, Q12, Q6, Q7, Q9 | [ ] | |
| CS-5 — Data-mode performance | E3, E1, E2, E5 | [ ] | |
| CS-6 — Runtime efficiency | E8, E9, E7, E4, E10 | [ ] | |
| CS-7 — Polish | R3, R8, Q11, Q13 | [ ] | |
