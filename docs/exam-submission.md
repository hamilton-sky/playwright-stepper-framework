<!--
  Historical document — preserved as written, not maintained.

  This is the original exam submission's design narrative, from the project's
  first week (10–14 April 2026). It is kept because it records *why* the
  framework is shaped the way it is, which nothing else in the repo does.

  It is reproduced verbatim, so its paths and commands are the ones that were
  true then. Four things have been renamed since:

      shared_poms/                        →  poms/
      exam/                               →  examples/plain_pom/
      cd stepper && python main.py …      →  python stepper/main.py …
      --workflow <path/to/file.json>      →  run <workflow-name>

  The only edit to the body is the two relative links at the end: they pointed
  at ARCHITECTURE.md and README.md from the repository root, and now point one
  level up from docs/. Same two files, same sentence — only the path had to
  follow the move.

  For what the framework is *now*, read docs/state-of-the-stepper.md.
-->
