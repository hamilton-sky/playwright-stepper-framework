...
| 3 | 3 | --apply-heals CLI — read suggestions, show diff, patch workflow JSON | DONE | `python stepper/main.py --apply-heals <workflow> --yes` patches file |
| 4 | 4 | StepRunner refactor — extract _run_step, _run_retry_loop, _run_heal_loop | DONE | `pytest exam/ -x -q` passes; `run()` ~50 lines |
...
| 3 | --apply-heals | Engine/CLI | Read suggestions, diff, patch workflow JSON | 3 | DONE | `stepper/main.py` |
| 4 | StepRunner Refactor | Engine | Extract _run_step/_run_retry_loop/_run_heal_loop; shrink run() to ~50 lines | 4 | DONE | `engine/runner/step_runner.py` |
